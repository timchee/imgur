export default function singleTag(
    ...
    let tagHeight = Math.floor((200 * aspectRatio) / 5);
    // let imageHeight = postHeight * 5;
    if (title.length > 115) {
      title = title.substring(0, 112);
      title += "...";
    }
    if (title.length > 78) tagHeight += 25;
    else if (title.length > 39) tagHeight += 22;
    else tagHeight += 20;
    ...
    //Posts' HTML
    let tag = `<div class="post text-white rounded-sm overflow-hidden w-[300px] w-[150px] tag-container bg-red" id="${id}" onClick="window.location ='pages/gallery.html?tagId=${id}'">
       <div class="w-[300px] bg-gradient-to-b from-tagColor-${gradientStartColor} to-gray-800" data-objectFit = "${objectFit}" style = "height: ${imageHeight}px"></div>
